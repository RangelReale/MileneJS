...
//
// Rangel Reale
// dynamic_pointer_cast support
//
template <class _Ty1, class _Ty2>
IntrusiveRefCntPtr<_Ty1> dynamic_pointer_cast(const IntrusiveRefCntPtr<_Ty2>& _Other) noexcept {
	typedef typename IntrusiveRefCntPtr<_Ty1>::element_type _Elem1;
	_Elem1 *_Ptr = dynamic_cast<_Elem1 *>(_Other.get());
	if (_Ptr)
		return (IntrusiveRefCntPtr<_Ty1>(_Ptr));
	else
		return (IntrusiveRefCntPtr<_Ty1>());
}

} // end namespace dtwrap
